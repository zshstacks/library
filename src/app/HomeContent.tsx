...
function CategoryCard({
  ...
  return (
    <Link
      href={`/${category.slug}`}
      className={`group block rounded-xl anim-fade-in-up focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#81e6d9]/60 focus-visible:ring-offset-2 focus-visible:ring-offset-[#202023] dark:focus-visible:ring-offset-[#f0e7db] ${DELAY_CLASSES[index] ?? "anim-delay-5"}`}
    >
      <div className="relative h-full rounded-xl border border-white/8 dark:border-gray-800/15 bg-white/[0.04] dark:bg-white/40 p-5 hover:shadow-lg hover:shadow-black/20 transition-all duration-300">
        <div
    ...
    <main className="min-h-screen text-white/80 dark:text-gray-800">
      <div className="max-w-[900px] mx-auto px-4 sm:px-6 pt-[88px] pb-16">
        ...
        >
          ...
          <p className="text-white/50 dark:text-gray-700/60 max-w-[500px] leading-relaxed text-[14px] mb-2">
          ...
          <p className="text-[12px] text-white/35 dark:text-gray-700/45">
        ...
        {topTags.length > 0 && (
          ...
          >
            <h2
              id="tags-heading"
              className="text-[11px] font-semibold uppercase tracking-widest text-white/28 dark:text-gray-700/38 mb-4"
            >
              Popular tags
            </h2>
            <div className="flex flex-wrap gap-2">
              {topTags.map((tag) => (
                <span
                  key={tag}
                  className="tag-item inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-white/45 dark:text-gray-700/50 bg-white/5 dark:bg-black/5 border border-white/8 dark:border-gray-800/12 text-[12px]"
                >
                  <BsTag size={10} aria-hidden="true" />
                  {tag}
                </span>
              ))}
            </div>
